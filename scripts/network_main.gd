...
func _configure_map() -> void:
	...
	var red_ground: float = terrain.height_at(red_center.x, red_center.y)
	var blue_ground: float = terrain.height_at(blue_center.x, blue_center.y)
	red_platform_surface_y = red_ground + clearance
	blue_platform_surface_y = blue_ground + clearance
	if bool(map_config["shared_platform_elevation"]):
		red_platform_surface_y = maxf(red_ground, blue_ground) + clearance
	...
	neutral_landmarks.configure(map_id, map_config, terrain)
	lobby.set_map_name(str(map_config["label"]))
	print("MAP selected id=%s label=%s" % [map_id, map_config["label"]])
...
func _on_peer_joined(id: int) -> void:
	...
	if multiplayer.is_server():
		...
		# Existing authoritative synchronizers can replicate before the joining
		# client's approval backfill RPC. A remote-only placeholder keeps those
		# paths valid; the local playable avatar remains agreement-gated.
		_spawn_avatar(id)
...
func get_team_spawn_transform(team: int, peer_id: int, respawn_count: int) -> Transform3D:
	...
	if not bool(socket["on_platform"]):
...
func get_bot_objective_position(peer_id: int) -> Vector3:
	...
	if _get_flag_carrier(enemy_team) == peer_id:
		...
		objective = red_home if player.team == TEAM_RED else blue_home
	...
	objective = _get_flag_position(enemy_team)
...
func _route_bot_toward(player_position: Vector3, objective: Vector3) -> Vector3:
	var waypoints := map_config["route_waypoints"] as Array
	if waypoints.size() <= 2:
		# Preserve a player's lateral lane on compact/direct maps without assuming
		# whether the team axis is X or Z.
		var axis := blue_home - red_home
		var planar_axis := Vector2(axis.x, axis.z).normalized()
		var lateral := Vector2(-planar_axis.y, planar_axis.x)
		var player_planar := Vector2(player_position.x, player_position.z)
		var objective_planar := Vector2(objective.x, objective.z)
		objective_planar += lateral * (player_planar - objective_planar).dot(lateral)
		return Vector3(objective_planar.x, objective.y, objective_planar.y)
	var player_planar := Vector2(player_position.x, player_position.z)
	var objective_planar := Vector2(objective.x, objective.z)
...
func _physics_process(_delta: float) -> void:
	...
	for avatar in avatars.values():
		...
		var oob := map_config["oob_half_extents"] as Vector2
		var outside := absf(position.x) > oob.x or absf(position.z) > oob.y
