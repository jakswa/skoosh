...
def create_materials(variant: Variant) -> dict[str, bpy.types.Material]:
    ...
    return {
        ...
        "ceramic": material("Weathered glacier ceramic", (0.34, 0.38, 0.38, 1.0), 0.04, 0.84),
        "hardware": material("Graphite mechanism", (0.047, 0.061, 0.064, 1.0), 0.58, 0.53),
        "team": material("Team shell seed", team_colors[variant.code], 0.08, 0.76),
        "mint": material(
            "Active mint channel",
            (0.025, 0.40, 0.32, 1.0),
            0.05,
        ...
        "bronze": material("Oxidized salvage bronze", (0.30, 0.17, 0.085, 1.0), 0.66, 0.61),
    }
...
def build_vector_core(add, mats: dict[str, bpy.types.Material]) -> None:
    ...
    add(
        front_panel_prism(
            ...
            0.25, mats["team"], 0.025,
        ),
        "Pelvis",
    )
    add(profile_prism("Helmet", (0, -0.01, 1.94), 0.15, [(-0.31, -0.15), (-0.35, 0.05), (-0.18, 0.18), (0.15, 0.13), (0.19, -0.14)], mats["team"], 0.027, (math.radians(-4), 0, 0)), "Head")
    ...
    for side, sign in (("L", 1.0), ("R", -1.0)):
        add(panel_prism(f"Shoulder fin {side}", (0, 0.015, 1.50), [(sign * 0.08, -0.10), (sign * 0.53, 0.01), (sign * 0.45, 0.12), (sign * 0.13, 0.065)], 0.065, mats["team"], 0.016, (math.radians(-7), 0, 0)), "Spine")
        ...
        add(rectangular_duct(f"Jet pod {side}", (pod_x, pod_y, pod_z), (0.30, 0.17), 0.36, 0.035, mats["team"], 0.016, (math.radians(-8), 0, 0)), "Spine")
...
def build_stratos_core(add, mats: dict[str, bpy.types.Material]) -> None:
    ...
    add(
        front_panel_prism(
            ...
            0.06, mats["team"], 0.022,
        ),
        "Spine",
    )
    add(front_panel_prism("Back frame", (0, 0.17, 1.37), [(-0.31, 0.24), (0.31, 0.24), (0.18, -0.28), (-0.18, -0.28)], 0.12, mats["hardware"], 0.022), "Spine")
    add(front_panel_prism("Rear bib", (0, 0.285, 1.43), [(-0.34, 0.19), (0.34, 0.19), (0.20, -0.22), (0, -0.29), (-0.20, -0.22)], 0.05, mats["team"], 0.014), "Spine")
    add(tapered_prism("Spine rail", (0, 0.34, 1.39), (0.055, 0.045, 0.48), (0.72, 0.66), mats["hardware"], 0.01), "Spine")
    add(front_panel_prism("Pelvis shell", (0, -0.015, 0.94), [(-0.27, 0.15), (0.27, 0.15), (0.18, -0.18), (-0.18, -0.18)], 0.23, mats["team"], 0.023), "Pelvis")
    add(profile_prism("Helmet", (0, -0.005, 1.95), 0.19, [(-0.25, -0.13), (-0.27, 0.05), (-0.12, 0.14), (0.14, 0.12), (0.18, -0.12)], mats["team"], 0.028, (math.radians(-6), 0, 0)), "Head")
    ...
    for side, sign in (("L", 1.0), ("R", -1.0)):
        add(panel_prism(f"Shoulder fin {side}", (0, 0.015, 1.59), [(0, -0.11), (sign * 0.68, -0.035), (sign * 0.60, 0.09), (0, 0.045)], 0.095, mats["team"], 0.017, (math.radians(-5), 0, 0)), "Spine")
        ...
        add(rectangular_duct(f"Jet pod {side}", (pod_x, pod_y, pod_z), (0.18, 0.44), 0.32, 0.034, mats["team"], 0.015, (math.radians(-7), 0, 0)), "Spine")
...
def build_khepri_core(add, mats: dict[str, bpy.types.Material]) -> None:
    ...
    add(front_panel_prism("Chest plate", (0, -0.16, 1.43), [(-0.30, 0.19), (0, 0.29), (0.30, 0.19), (0.20, -0.13), (0, -0.26), (-0.20, -0.13)], 0.06, mats["team"], 0.02), "Spine")
    ...
    add(front_panel_prism("Rear bib", (0, 0.305, 1.41), [(-0.20, 0.17), (0, 0.25), (0.20, 0.17), (0.14, -0.19), (0, -0.25), (-0.14, -0.19)], 0.045, mats["team"], 0.012), "Spine")
    add(tapered_prism("Spine rail", (0, 0.35, 1.38), (0.085, 0.05, 0.47), (0.38, 0.70), mats["bronze"], 0.012), "Spine")
    add(front_panel_prism("Pelvis shell", (0, -0.01, 0.94), [(-0.29, 0.13), (0, 0.21), (0.29, 0.13), (0.20, -0.12), (0, -0.20), (-0.20, -0.12)], 0.27, mats["team"], 0.024), "Pelvis")
    add(front_panel_prism("Helmet", (0, -0.01, 1.95), [(-0.22, -0.12), (-0.08, 0.17), (0.14, 0.13), (0.23, -0.02), (0.10, -0.16)], 0.37, mats["team"], 0.027, (math.radians(-7), 0, 0)), "Head")
    ...
    for side, sign in (("L", 1.0), ("R", -1.0)):
        ...
        add(panel_prism(f"Shoulder fin {side}", (0, 0.01, 1.56), [(sign * inner, -0.12), (sign * reach, -0.035), (sign * (reach - 0.09), 0.10), (sign * 0.14, 0.055)], 0.07, mats["team"], 0.016, (math.radians(-7), 0, math.radians(sign * 2))), "Spine")
        ...
        add(rectangular_duct(f"Jet pod {side}", (pod_x, pod_y, pod_z), (0.22, 0.29), 0.33, 0.035, mats["team"], 0.015, (math.radians(-9), 0, math.radians(sign * 3))), "Spine")
...
def build_connected_limbs(variant: Variant, add, mats: dict[str, bpy.types.Material]) -> None:
    ...
    for side, sign in (("L", 1.0), ("R", -1.0)):
        ...
        add(front_panel_prism(f"Thigh signal {side}", (sign * 0.18, -0.15, 0.70), [(-0.055, 0.11), (0.055, 0.08), (0.045, -0.12), (-0.045, -0.09)], 0.025, mats["team"], 0.008), f"Thigh.{side}")
...
def save_export_and_report(variant: Variant) -> None:
    ...
    print(
        f"CHARACTER_SHELL {variant.slug}: meshes={len(meshes)} vertices={vertices} "
        f"triangles={triangles} materials={len(bpy.data.materials)} bones={len(BONE_CONTRACT)} "
...
def build_variant(variant: Variant) -> None:
    ...
    save_export_and_report(variant)
